...
| --------------- | --------------- | --------- | ------------ |
| Angular 15      | >= v15.0.0      | `latest`  | ✅ Supported |
...
```bash
npm install @ngx-cocktail/common@ng15
npm install @ngx-cocktail/destroyable@ng15
npm install @ngx-cocktail/title@ng15
```
...
Each release branch (`release/vN`) maps to one Angular major version. npm **dist-tags** (`latest`, `ng16`, `ng15`, …) let users install the right version for their Angular version without knowing exact patch numbers.
...
| `ng16`   | latest patch for v16  | Angular 16 users pinned to that tag |
| `ng16`   | latest patch for v16  | Angular 16 users pinned to that tag |
| …        | …                     | …                                   |
...
# --tag ng15 prevents overwriting "latest"
cd dist/common && npm publish --tag ng15 --access public && cd ../..
cd dist/destroyable && npm publish --tag ng15 --access public && cd ../..
cd dist/title && npm publish --tag ng15 --access public && cd ../..
```

### Adding a dist-tag to an already-published version

```bash
npm dist-tag add @ngx-cocktail/common@15.0.1 ng15
npm dist-tag add @ngx-cocktail/destroyable@15.0.1 ng15
npm dist-tag add @ngx-cocktail/title@15.0.1 ng15
```
